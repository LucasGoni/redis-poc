...
import { redisClient, isConnectedToRedis } from '../redis/redisClient';
...
router.get('/', validateSearchParams, async (req: Request, res: Response) => {
  ...
  try {
    if (!isConnectedToRedis) {
      console.log('Redis is not connected. Skipping cache.');
    }
    ...
    const apiElapsedMilliseconds: number = calculateElapsedMilliseconds(apiStartTime, apiEndTime);
